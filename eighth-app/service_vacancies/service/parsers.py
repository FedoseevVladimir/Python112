...
__all__ = ('hh', 'superjobs')

# url = 'https://novosibirsk.hh.ru/search/vacancy?text=python&from=suggest_post&area=1'
    ...
    if resp.status_code == 200:
        ...
        if main_div:
            ...
            for div in div_lst:
                ...
                jobs.append({'title': title, 'url': href, 'desc': content, 'company': company})
        else:
...
def superjobs(url):
    ...
    if resp.status_code == 200:
        ...
        if main_div:
            ...
            for div in div_lst:
                tit = div.find('div', _class='_2J-3z')
                if tit:
                ...
                jobs.append({'title': title, 'url': domain + str(href), 'desc': str(content), 'company': company})
        else:
            errors.append({'url': url, 'title': 'Разметка была изменена'})
    else:
        errors.append({'url': url, 'title': 'Страница не отвечает'})
